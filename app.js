...
function saveAdminPhotos(items){
    const clean = Array.from(items || []).filter(Boolean).slice(0, 50);

    try{
        localStorage.setItem("livia_extra_photos", JSON.stringify(clean));
        return true;
    }catch(error){
        console.warn("Limite de fotos/localStorage atingido:", error);

        // Mantém as fotos mais recentes se o navegador não aceitar todas.
        for(let limit of [40, 30, 20, 12, 8, 4]){
            try{
                localStorage.setItem("livia_extra_photos", JSON.stringify(clean.slice(0, limit)));
                showToast(`Memória cheia: salvei as ${limit} fotos mais recentes.`, "error");
                return true;
            }catch(e){}
        }

        showToast("Memória cheia. Exclua fotos antigas e tente novamente.", "error");
        return false;
    }
}
...
function compressPhotoSource(src, maxSize = 720, quality = 0.68){
    return new Promise(resolve => {
        if(!src || typeof src !== "string") return resolve(null);

        // Só compacta imagens grandes em base64. Assets padrão continuam iguais.
        if(!src.startsWith("data:image/") || src.length < 180000){
            return resolve(src);
        }
...
function compressPhotoFile(file, maxSize = 720, quality = 0.68){
    return new Promise(resolve => {
...
async function optimizeStoredAdminPhotos(){
    const current = getAdminPhotos();
    if(current.length === 0) return [];

    const optimized = [];
    for(const photo of current){
        const fixed = await compressPhotoSource(photo);
        if(fixed) optimized.push(fixed);
    }

    saveAdminPhotos(optimized);
    return optimized;
}

async function addAdminPhoto(file, skipRender = false){
...
async function addAdminPhotos(files){
    const selected = Array.from(files || []).filter(file => file && file.type && file.type.startsWith("image/"));
    ...
    showToast("Carregando foto(s)...");

    // Compacta as fotos antigas primeiro. Isso resolve quando o app trava em 2 fotos por falta de memória.
    await optimizeStoredAdminPhotos();
...
document.getElementById("btnAddAdminPhoto").addEventListener("click", () => {
    const input = document.getElementById("adminPhotoInput");
    input.setAttribute("multiple", "multiple");
    input.multiple = true;
    input.click();
});
const adminPhotoInputV43 = document.getElementById("adminPhotoInput");
if(adminPhotoInputV43){
    adminPhotoInputV43.setAttribute("multiple", "multiple");
    adminPhotoInputV43.multiple = true;
    adminPhotoInputV43.addEventListener("change", async event => {
...
/* ===== V41 FOTOS: permitir várias fotos no gerenciador ===== */
(function injectLiviaV41PhotoStyles(){
    const styleId = "livia_v41_photo_styles";
    if(document.getElementById(styleId)) return;
...
function forceAdminPhotoMultipleV41(){
    const input = document.getElementById("adminPhotoInput");
    if(input){
        input.setAttribute("multiple", "multiple");
        input.multiple = true;
...
setInterval(forceAdminPhotoMultipleV41, 1000);
document.addEventListener("DOMContentLoaded", forceAdminPhotoMultipleV41);



/* ===== v44: segurança para Pix não voltar ao botão CONCLUIR ===== */
(function fixPixButtonTextV44(){
    function apply(){
        const btn=document.getElementById('btnFinish');
        if(btn) btn.textContent='🗑️ DESISTIR DA RESERVA';
    }

...
function showPrettyConfirm({ title, message, confirmText = "CONFIRMAR", cancelText = "CANCELAR", danger = false }){
    return new Promise(resolve => {
        ...
        if(!modal){
            ...
            style.textContent = `
                #prettyConfirmModal{position:fixed;inset:0;z-index:99999;display:none;align-items:center;justify-content:center;padding:22px;background:rgba(0,0,0,.72);backdrop-filter:blur(8px)}
                #prettyConfirmModal.active{display:flex}
                .pretty-confirm-box{width:min(92vw,460px);border:1px solid rgba(255,105,180,.55);border-radius:28px;padding:26px 22px 22px;background:linear-gradient(180deg,#2a0718,#15000b);box-shadow:0 24px 70px rgba(0,0,0,.55);color:#fff;text-align:center}
                .pretty-confirm-icon{width:66px;height:66px;margin:0 auto 12px;border-radius:999px;display:flex;align-items:center;justify-content:center;background:rgba(255,105,180,.14);border:1px solid rgba(255,105,180,.45);font-size:30px}
                .pretty-confirm-box h2{margin:0 0 10px;font-size:26px;line-height:1.12;color:#ff69b4;font-weight:900}
                .pretty-confirm-box p{margin:0 auto 22px;font-size:17px;line-height:1.45;color:#ead6df;max-width:360px;white-space:pre-line}
                .pretty-confirm-actions{display:grid;grid-template-columns:1fr 1fr;gap:12px}
                .pretty-confirm-actions button{min-height:52px;border-radius:18px;border:0;font-size:15px;font-weight:900;letter-spacing:.2px}
                #prettyConfirmCancel{background:#2a1020;color:#fff;border:1px solid rgba(255,255,255,.12)}
                #prettyConfirmOk{background:linear-gradient(135deg,#e45a9d,#9a1230);color:#fff;box-shadow:0 12px 24px rgba(228,90,157,.18)}
...
async function cleanOldAppointments(){
    ...
    const ok = await showPrettyConfirm({
        ...
        message: `Você vai apagar ${removable.length} agendamento(s) expirado(s)/cancelado(s).\n\nConfirmados e reservas temporárias serão mantidos.`,

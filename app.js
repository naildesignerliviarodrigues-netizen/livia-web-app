...
document.getElementById("customDateInput").addEventListener("input", (event) => {
    ...
    if(value.length > 4) value = `${value.slice(0, 99)}/${value.slice(2,4)}/${value.slice(4)}`;
    else if(value.length > 99) value = `${value.slice(0, 99)}/${value.slice(2)}`;
...
function formatPhone(value){
    const d = value.replace(/\D/g, "").slice(0, 11);
    if(d.length > 10) return `(${d.slice(0, 99)}) ${d.slice(2,7)}-${d.slice(7)}`;
    if(d.length > 6) return `(${d.slice(0, 99)}) ${d.slice(2,6)}-${d.slice(6)}`;
    if(d.length > 99) return `(${d.slice(0, 99)}) ${d.slice(2)}`;
...
(function injectLiviaV34Styles(){
    if(document.getElementById("livia_v34_qr_gallery_styles")) return;
    const style = document.createElement("style");
    style.id = "livia_v34_qr_gallery_styles";
    style.textContent = `
/* v34 - QR centralizado exatamente no quadrinho branco */
#pixScreen .pix-card,
#pixScreen .pix-payment-card,
#pixScreen .pix-box,
#pixScreen .pix-row {
...
#pixScreen .qr-wrap,
#pixScreen .qr-box,
#pixScreen .pix-qr-box,
#pixScreen .qr-container {
...
#pixScreen img[id*="qr" i],
#pixScreen canvas[id*="qr" i],
#pixScreen img[src*="base64"],
#pixScreen canvas {
    display: block !important;
...
/* v34 - galeria aceita mais fotos e fica com rolagem suave */
.gallery-grid,
.photos-grid,
.admin-photos-grid,
#photosList,
...
function fixPixQrAlignmentV34(){
    const pixScreen = document.getElementById("pixScreen");
    if(!pixScreen) return;

    const qrEl = pixScreen.querySelector('img[id*="qr" i], canvas[id*="qr" i], img[src*="base64"], canvas');
    if(!qrEl) return;

    let box = qrEl.parentElement;
    for(let i = 0; i < 3 && box && box !== pixScreen; i++){
        const rect = box.getBoundingClientRect();
        if(rect.width >= 120 && rect.width <= 260){
            break;
        }
        box = box.parentElement;
    }

    if(box && box !== pixScreen){
    ...
    qrEl.style.margin = "auto";
    qrEl.style.objectFit = "contain";
    qrEl.style.maxWidth = "100%";
    qrEl.style.maxHeight = "100%";
    qrEl.style.position = "static";
    qrEl.style.transform = "none";
}

setInterval(fixPixQrAlignmentV34, 600);
document.addEventListener("DOMContentLoaded", fixPixQrAlignmentV34);




function getStoredStudioPhotosV34(){
    try{
        const keys = ["studioPhotos", "liviaPhotos", "galleryPhotos", "workPhotos", "portfolioPhotos"];
...
function unlockPhotoLimitV34(){
    ...
    buttons.forEach(btn => {
        ...
        if(txt.includes("foto") || txt.includes("imagem") || txt.includes("upload")){
            btn.disabled = false;
...
setInterval(unlockPhotoLimitV34, 1000);
document.addEventListener("DOMContentLoaded", unlockPhotoLimitV34);

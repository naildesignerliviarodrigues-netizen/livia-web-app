...
const business = { ...BUSINESS_DEFAULTS };
...
function isLunchBlocked(start, end, date = selectedDateObj){
    // Sábado tem atendimento contínuo, sem intervalo para almoço.
    if(date && date.getDay && date.getDay() === 6){
        return false;
    }
    if(!business.lunchEnabled) return false;
...
function generateTimesForService(service){
    ...
    const addRange = (rangeStart, rangeEnd) => {
        for(let current = rangeStart; current + service.durationMinutes <= rangeEnd; current += step){
            times.push(minutesToTime(current));
        }
    };

    if(!business.lunchEnabled || isSaturday){
        addRange(start, end);
        return times;
    }

    const lunchStart = timeToMinutes(business.lunchStart);
    const lunchEnd = timeToMinutes(business.lunchEnd);

    // Antes do almoço: só entra o que termina até o início da pausa.
    addRange(start, Math.min(lunchStart, end));

    // Depois do almoço: a grade reinicia exatamente no fim da pausa.
    if(lunchEnd < end){
        addRange(Math.max(lunchEnd, start), end);
    }
...
function normalizeBusinessSettings(data){
    if(!data || typeof data !== "object") return { ...BUSINESS_DEFAULTS };
    return {
        start: /^\d{2}:\d{2}$/.test(data.start || "") ? data.start : BUSINESS_DEFAULTS.start,
        weekdayEnd: /^\d{2}:\d{2}$/.test(data.weekdayEnd || "") ? data.weekdayEnd : BUSINESS_DEFAULTS.weekdayEnd,
        saturdayEnd: /^\d{2}:\d{2}$/.test(data.saturdayEnd || "") ? data.saturdayEnd : BUSINESS_DEFAULTS.saturdayEnd,
        lunchEnabled: data.lunchEnabled !== false,
        lunchStart: /^\d{2}:\d{2}$/.test(data.lunchStart || "") ? data.lunchStart : BUSINESS_DEFAULTS.lunchStart,
        lunchEnd: /^\d{2}:\d{2}$/.test(data.lunchEnd || "") ? data.lunchEnd : BUSINESS_DEFAULTS.lunchEnd,
        daysAhead: Math.min(60, Math.max(1, Number(data.daysAhead) || BUSINESS_DEFAULTS.daysAhead))
    };
}

function applyBusinessSettings(settings){
    Object.assign(business, normalizeBusinessSettings(settings));
    renderBusinessSettingsAdmin();
    if(document.querySelector(".screen.active")?.id === "dateScreen" && selectedService) renderDates();
    if(document.querySelector(".screen.active")?.id === "slotsScreen" && selectedService && selectedDateObj) renderTimes();
}

async function fetchBusinessSettingsFromFirebase(){
    try{
        const response = await fetch(firebaseUrl("settings/business"), { cache: "no-store" });
        if(!response.ok) throw new Error(`Firebase HTTP ${response.status}`);
        const data = await response.json();
        applyBusinessSettings(data || BUSINESS_DEFAULTS);
        return business;
    }catch(error){
        console.warn("Configurações da agenda indisponíveis; usando padrão local:", error);
        applyBusinessSettings(BUSINESS_DEFAULTS);
        return business;
    }
}

async function saveBusinessSettingsToFirebase(settings){
    const normalized = normalizeBusinessSettings(settings);
    const response = await fetch(firebaseUrl("settings/business"), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(normalized)
    });
    if(!response.ok) throw new Error(`Firebase PUT ${response.status}`);
    applyBusinessSettings(normalized);
    return normalized;
}

function renderBusinessSettingsAdmin(){
    const ids = {
        adminBusinessStart: business.start,
        adminWeekdayEnd: business.weekdayEnd,
        adminSaturdayEnd: business.saturdayEnd,
        adminLunchStart: business.lunchStart,
        adminLunchEnd: business.lunchEnd,
        adminDaysAhead: business.daysAhead
    };
    Object.entries(ids).forEach(([id, value]) => {
        const el = document.getElementById(id);
        if(el && document.activeElement !== el) el.value = value;
    });
    const lunchToggle = document.getElementById("adminLunchEnabled");
    if(lunchToggle) lunchToggle.checked = !!business.lunchEnabled;
    const lunchFields = document.getElementById("adminLunchFields");
    if(lunchFields) lunchFields.classList.toggle("disabled", !business.lunchEnabled);
}

function readBusinessSettingsAdmin(){
    return {
        start: document.getElementById("adminBusinessStart")?.value || business.start,
        weekdayEnd: document.getElementById("adminWeekdayEnd")?.value || business.weekdayEnd,
        saturdayEnd: document.getElementById("adminSaturdayEnd")?.value || business.saturdayEnd,
        lunchEnabled: !!document.getElementById("adminLunchEnabled")?.checked,
        lunchStart: document.getElementById("adminLunchStart")?.value || business.lunchStart,
        lunchEnd: document.getElementById("adminLunchEnd")?.value || business.lunchEnd,
        daysAhead: Number(document.getElementById("adminDaysAhead")?.value || business.daysAhead)
    };
}

function validateBusinessSettings(settings){
    const start = timeToMinutes(settings.start);
    const weekdayEnd = timeToMinutes(settings.weekdayEnd);
    const saturdayEnd = timeToMinutes(settings.saturdayEnd);
    if(start >= weekdayEnd || start >= saturdayEnd) return "O horário de abertura precisa ser antes do fechamento.";
    if(settings.lunchEnabled){
        const lunchStart = timeToMinutes(settings.lunchStart);
        const lunchEnd = timeToMinutes(settings.lunchEnd);
        if(lunchStart >= lunchEnd) return "O início do almoço precisa ser antes do fim.";
        if(lunchStart < start || lunchEnd > weekdayEnd) return "O almoço precisa ficar dentro do expediente de terça a sexta.";
    }
    if(settings.daysAhead < 1 || settings.daysAhead > 60) return "Use de 1 a 60 dias disponíveis para agendamento.";
    return "";
}

async function handleSaveBusinessSettings(){
    const btn = document.getElementById("btnSaveBusinessSettings");
    const settings = readBusinessSettingsAdmin();
    const error = validateBusinessSettings(settings);
    if(error){
    ...
    try{
        ...
        await saveBusinessSettingsToFirebase(settings);
        showToast("Horários salvos. A agenda já foi reorganizada ✓");
    }catch(error){
        console.warn("Erro salvando configurações:", error);
        showToast("Não consegui salvar no Firebase.", "error");
    }finally{
        if(btn){ btn.disabled = false; btn.textContent = "SALVAR HORÁRIOS"; }
    }
}
...
renderServices();
renderHomeWorks();
fetchBusinessSettingsFromFirebase();
startAppointmentsRealtime();
setupInfiniteWorksCarousel();
setupHiddenAdminAccess();
bindRealPixButton();

const btnSaveBusinessSettings = document.getElementById("btnSaveBusinessSettings");
if(btnSaveBusinessSettings) btnSaveBusinessSettings.addEventListener("click", handleSaveBusinessSettings);
const adminLunchEnabled = document.getElementById("adminLunchEnabled");
if(adminLunchEnabled){
    adminLunchEnabled.addEventListener("change", () => {
        const lunchFields = document.getElementById("adminLunchFields");
        if(lunchFields) lunchFields.classList.toggle("disabled", !adminLunchEnabled.checked);
    });
}

...
async function desistirReservaPixAtual(){
    if(!currentBookingId){
        showScreen("home");
        return;
    }
    ...
    const items = getStoredAppointments().map(item => {
        if(item.id === id){
            return {
                ...
                paymentStatus: "cancelled_by_client"
            };
        }
        return item;
    });
    ...
    try{
        await patchAppointmentFirebase(id, {
            status: "cancelado",
            cancelled_at: new Date().toLocaleString("pt-BR"),
            payment_status: "cancelled_by_client",
            admin_seen: false
        ...
        console.warn("Erro cancelando reserva Pix no Firebase:", error);
    }
    ...
    loadAppointmentsFromFirebase();
    showScreen("home");
    showToast("Reserva cancelada. O horário voltou a ficar disponível.", "warning");
}
...
function adminBookingCard(item){
    ...
    const canConfirm = false; // Pagamento agora é automático pelo Asaas/webhook

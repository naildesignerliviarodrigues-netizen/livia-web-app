...
document.getElementById("btnCopyPix").addEventListener("click", async () => {
    try{
        ...
        alert("Não consegui copiar automaticamente. Toque e segure para copiar quando o Pix real for ativado.");
    }
});
...
document.getElementById("btnFinish").addEventListener("click", () => {
    ...
    document.getElementById("successDateTime").textContent = selectedDateBR && selectedTime ? `${selectedDateBR} às ${selectedTime}` : "---";
    showScreen("success");
});
...
function adminBookingCard(item){
    ...
    const canConfirm = isStatus(item, "temporary") || isStatus(item, "expired");
    ...
    return `
        <section class="admin-booking-card ${statusClass(status)}">
            ...
            <div class="admin-actions-row">
                ...
                ${canConfirm ? `<button class="admin-confirm-btn" data-id="${item.id}">CONFIRMAR PAGAMENTO</button>` : ""}
...
function renderAdmin(){
    ...
    list.querySelectorAll(".admin-confirm-btn").forEach(btn => {
        btn.addEventListener("click", () => updateAppointmentStatus(btn.dataset.id, "Confirmado"));
    });
...
async function generatePixAndReserveReal(){
    try{
        ...
        const pixPayload = payData.pix_payload || payData.pixPayload || "";
        ...
        clientData.pixPayload = pixPayload;
        clientData.encodedImage = encodedImage;
        clientData.invoiceUrl = invoiceUrl;

        fillPix();
        setPixQRCode(pixPayload, encodedImage);
        hideLoading();
        showScreen("pix");
        startPixCountdown();
        loadAppointmentsFromFirebase();

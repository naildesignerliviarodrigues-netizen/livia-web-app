...
(function injectLiviaV35SafeStyles(){
  if(document.getElementById("livia_v35_safe_fixes")) return;
  const style = document.createElement("style");
  style.id = "livia_v35_safe_fixes";
  style.textContent = `
/* v35: centralização segura do QR dentro do quadrinho branco */
#pixScreen .pix-qr-wrap,
#pixScreen .qr-wrap,
#pixScreen .qr-box,
...
#pixScreen .qr-box img,
#pixScreen .pix-qr-box img,
#pixScreen .qr-wrap img {
  display: block !important;
...
function fixPixQrAlignmentV35(){
  const pixScreen = document.getElementById("pixScreen");
  if(!pixScreen) return;

  const qr = pixScreen.querySelector('canvas[id*="qr" i], img[id*="qr" i], .qr-box canvas, .qr-box img, .pix-qr-box canvas, .pix-qr-box img, .qr-wrap canvas, .qr-wrap img');
  if(!qr) return;
  ...
  qr.style.maxWidth = "92%";
  qr.style.maxHeight = "92%";
  qr.style.width = "auto";
...
setInterval(fixPixQrAlignmentV35, 700);
document.addEventListener("DOMContentLoaded", fixPixQrAlignmentV35);




function enableMultipleStudioPhotosV35(){
  const admin = document.getElementById("photosScreen") || document.getElementById("adminPhotosScreen") || document;
  const inputs = Array.from(admin.querySelectorAll('input[type="file"]'));
  inputs.forEach(input => {
    const nearby = (input.id + " " + input.name + " " + (input.closest("section,div")?.innerText || "")).toLowerCase();
    if(nearby.includes("foto") || nearby.includes("photo") || nearby.includes("trabalho") || nearby.includes("imagem")){
      input.multiple = true;
...
setInterval(enableMultipleStudioPhotosV35, 1000);
document.addEventListener("DOMContentLoaded", enableMultipleStudioPhotosV35);

...
function buildReminderMessage(item){
    const nome = item.clientName || "cliente";
    const data = item.date || "--/--/----";
    const horario = item.time || "--:--";
    const servico = item.service || "seu atendimento";

    return [
        `Olá, ${nome}! 💅`,
        "",
        "Aqui é o Studio Lívia Rodrigues.",
        "",
        ...
        "Até breve! 💖",
        "",
        "Studio Lívia Rodrigues"
    ].join("\n");
}

function ensureAdminReminderStyle(){
    if(document.getElementById("adminReminderStyle")) return;

    const style = document.createElement("style");
    style.id = "adminReminderStyle";
    style.textContent = `
        ...
        .admin-reminder-btn{
            background:linear-gradient(135deg,#e45a9d,#ff78bb)!important;
            color:#14000b!important;
        }
...
function adminBookingCard(item){
    ...
    const reminderText = buildReminderMessage(item);
    const reminderLink = phoneDigits ? `https://wa.me/55${phoneDigits}?text=${encodeURIComponent(reminderText)}` : "#";
        ...
        <section class="admin-booking-card ${statusClass(status)}">
            ...
            ${phoneDigits ? `
                <div class="admin-contact-actions">
                    <button class="admin-whatsapp-btn" data-whatsapp="${waLink}">WHATSAPP</button>
                    <button class="admin-reminder-btn" data-whatsapp="${reminderLink}">LEMBRETE</button>
                </div>
            ` : ""}
...
function renderAdmin(){
    ...
    list.querySelectorAll(".admin-reminder-btn").forEach(btn => {
        btn.addEventListener("click", () => window.open(btn.dataset.whatsapp, "_blank"));
    });
